...
_logo_path = Path(__file__).parent / "assets" / "lacoste_logo.png"
if _logo_path.exists():
    _logo_b64 = base64.b64encode(_logo_path.read_bytes()).decode()
    _logo_html = (
        f'<img src="data:image/png;base64,{_logo_b64}" '
        f'style="height:80px; margin-bottom:8px;" alt="logo">'
    )
else:
    _logo_html = ""
...
st.markdown(
    ...
    {_logo_html}
    <h1 style="color:#006A4E; margin-bottom:4px;">{APP_TITLE}</h1>
...
with col_v:
    st.markdown(
        f"""
        <div style="border-left: 4px solid {VERTEX_COLOR}; background: #F5FAF7;
...
with col_h:
    st.markdown(
        f"""
        <div style="border-left: 4px solid {HYBRID_COLOR}; background: #F5FAF7;
             border-radius: 0 8px 8px 0; padding: 12px 16px;">
