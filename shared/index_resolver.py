...
def resolve_indexes(query: str, available: list[str]) -> list[str]:
    ...
    prompt = (
        ...
        "misfiled documents may live there.\n"
        "5. When in doubt, return MORE indexes — better to search too "
        "broadly than to miss information.\n"
        "6. NEVER pick an index just because a word loosely overlaps "
        "with its label (e.g. 'budget' must NOT route to 'patrimoine-"
        "maintenance-énergie' just because the label contains 'énergie').\n\n"
